...
resource "dynatrace_json_dashboard" "comos_rds_dasboard" {
  contents = jsonencode(
    {
      "dashboardMetadata" : {
        "name" : "comos_rds_dasboard_terraform",
        "shared" : true,
        "owner" : "Dynatrace",
        "tags" : [
          "cosmos"
    ...
    },
    {
      ...
      "metricExpressions": [
        "resolution=Inf&(ext:cloud.aws.rds.cpuUtilization:splitBy():sort(value(auto,descending)):limit(20)):limit(100):names",
        "resolution=null&(ext:cloud.aws.rds.cpuUtilization:splitBy():sort(value(auto,descending)):limit(20))"
      ]
    },
    {
